...
/root/go/bin/derper --hostname $TAILSCALE_DERP_HOSTNAME --bootstrap-dns-names $TAILSCALE_DERP_HOSTNAME -certmode $TAILSCALE_DERP_CERTMODE -certdir /root/derper/$TAILSCALE_DERP_HOSTNAME --a=$DERP_ADDR --http-port=$DERP_HTTP_PORT --stun --verify-clients=$TAILSCALE_DERP_VERIFY_CLIENTS
